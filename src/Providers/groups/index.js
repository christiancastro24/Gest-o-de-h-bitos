...
export const GroupsProvider = ({ children }) => {
  ...
  const handleCreateGoal = (itemId) => {
    ...
    api
      ...
      .catch((_) =>
        toast.error("Erro ao cadastrar uma meta", {
          style: {
            backgroundColor: "red",
            color: "#fff",
          },
  ...
  const handleCreateActivity = (itemId) => {
    ...
    api
      .post(
        "/activities/",
        { ...dataActivities, realization_time },
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleLogout = (chave) => {
    ...
    setLoading(true);
    api
      .delete(`/groups/${chave}/unsubscribe/`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
        setLoading(false);
